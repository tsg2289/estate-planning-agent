...
CREATE INDEX IF NOT EXISTS idx_estate_documents_user_id ON estate_documents(user_id);
CREATE INDEX IF NOT EXISTS idx_estate_documents_type ON estate_documents(document_type);
CREATE INDEX IF NOT EXISTS idx_estate_documents_status ON estate_documents(status);
CREATE INDEX IF NOT EXISTS idx_email_list_subscribed ON email_list(subscribed) WHERE subscribed = true;
...
-- Drop existing policies for estate_documents
...
-- Drop existing policies for email_list
...
-- Drop existing policies for document_templates
...
-- Insert some sample data (only if templates don't exist)
INSERT INTO document_templates (document_type, name, description, content, created_by) 
SELECT 'will', 'Basic Will Template', 'A simple will template for basic estate planning', 
 '{"sections": ["executor", "beneficiaries", "assets", "guardianship"]}', NULL
WHERE NOT EXISTS (SELECT 1 FROM document_templates WHERE document_type = 'will');

INSERT INTO document_templates (document_type, name, description, content, created_by) 
SELECT 'trust', 'Revocable Living Trust', 'A comprehensive revocable living trust template',
 '{"sections": ["trustee", "beneficiaries", "assets", "distribution"]}', NULL
WHERE NOT EXISTS (SELECT 1 FROM document_templates WHERE document_type = 'trust');

INSERT INTO document_templates (document_type, name, description, content, created_by) 
SELECT 'poa', 'Durable Power of Attorney', 'Power of attorney for financial matters',
 '{"sections": ["agent", "powers", "limitations", "successor"]}', NULL
WHERE NOT EXISTS (SELECT 1 FROM document_templates WHERE document_type = 'poa');

INSERT INTO document_templates (document_type, name, description, content, created_by) 
SELECT 'ahcd', 'Advance Health Care Directive', 'Health care directive and living will',
 '{"sections": ["healthcare_agent", "treatment_preferences", "end_of_life"]}', NULL
WHERE NOT EXISTS (SELECT 1 FROM document_templates WHERE document_type = 'ahcd');

-- Create a view for user dashboard data (drop first if exists)
